...
@interface MTEditPopView ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate,UIPickerViewDataSource,UIPickerViewDelegate,UITextFieldDelegate> {
    ...
    NSMutableArray * _selectCallArray;
}
...
- (instancetype _Nonnull)initWithType:(MTEditPopViewType)popViewType delegate:(id)delegate {
    if (self = [super init]) {
        ...
        _selectCallArray = [[NSMutableArray alloc]init];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.popViewType == MTEditPopViewText) {
        ...
        if (_iniDate) {
            [picker setDate:_iniDate animated:NO];
        }
        else{
        ...
        if (_iniDate) {
            ...
            NSString *strHour = [dateFormatter stringFromDate:_iniDate];
            [dateFormatter setDateFormat:@"mm"];
            NSString *strMin = [dateFormatter stringFromDate:_iniDate];
            
            [picker2 selectRow:[strHour integerValue] inComponent:0 animated:NO];
            
            [picker2 selectRow:[strMin integerValue] inComponent:1 animated:NO];
...
- (void)setupButtomView {
   ...
   // [_confirmBtn setBackgroundImage:[UIImage imageWithColor:secondColor] forState:UIControlStateNormal];
   // [_confirmBtn setBackgroundImage:[UIImage imageWithColor:firstColor] forState:UIControlStateHighlighted];
   ...
   // [_cancelBtn setBackgroundImage:[UIImage imageWithColor:secondColor] forState:UIControlStateNormal];
    //[_cancelBtn setBackgroundImage:[UIImage imageWithColor:firstColor] forState:UIControlStateHighlighted];
    [_cancelBtn addTarget:self action:@selector(cancelBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [_bottomView addSubview:_cancelBtn];
    
    if(self.popViewType==MTEditPopViewCall){
        
         [_confirmBtn setTitle:@"联系" forState:UIControlStateNormal];
        [_cancelBtn setTitle:@"不联系" forState:UIControlStateNormal];
    }
...
//    }
    if(self.popViewType==MTEditPopViewCall){
        NSArray *textarr=@[@"联系电话1:",@"联系电话2:",@"接入号码:",@"手动输入:"];
        ...
        if ([_selectCallArray containsObject:textarr[indexPath.row]]) {
        ...
        if(indexPath.row==3){
            ...
            if(!_textfield){
                ...
                _textfield=[[UITextField alloc]initWithFrame:CGRectMake(110, 0, 120, 40)];
    ...
    if (self.popViewType == MTEditPopViewMultiple) {
        ...
        [_resultArray addObject:cellTitle];
         ...
         if (self.popViewType == MTEditPopViewCall) {
             ...
             [_selectCallArray removeAllObjects];
             [_selectCallArray addObject:textarr[indexPath.row]];
         }
    }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    [_dataSources replaceObjectAtIndex:3 withObject:textField.text];
    return YES;
}
